...
function onCreate()
	...
	-- background shit
	makeLuaSprite('sky', 'Camp SW/SkyBox', -1620, -1000);
	...
	makeLuaSprite('houses', 'Camp SW/CabinAndFloor', -1790, -900);
	...
	if not lowQuality then
		...
		makeAnimatedLuaSprite('doorOpen', 'Camp SW/DoorOpen', 610, 284); -- from that door on Hard comes up Matt with Tom
		...
		makeLuaSprite('city', 'Camp SW/SecondParalax', -1650, -700);
...
function onBeatHit()
	...
	if curBeat == 398 then -- cutscene ending
		doTweenAlpha('hudFunne', 'camHUD', 1, 0.4, 'linear') -- ALEX DIRECTOR WROTE THIS WOOO-
	end
end
...
function onCreatePost()
	if not hideHud then
	makeLuaSprite('eddI', 'redsm', getProperty('iconP1.x'), getProperty('iconP1.y'))
